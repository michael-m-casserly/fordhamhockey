...
var iEmail = "A valid email address is required."
...
var pEmail = "valid email address (like johndoe@abc123.com)."
...
{   var i;
    ...
    {   
        ...
        var c = s.charAt(i);
		// allows for letters, blank spaces, apostrophes, dashes, commas, and periods...these are for Proper Name fields
...
function checkAlpabets(fld, s, emptyOK) {
	var valid = false;
	valid = checkString(fld, s);
	if (valid) {
		valid = isAlphabetic(fld.value);
		if (!valid) {	
			alert(s + " must only contain letters, dashes, apostrophes, commas or periods.");
			fld.focus();
		}
	}
...
function checkNumber(fld, s, size, emptyOK) {
	var valid = false;
	var ldefaultEmtpyOK = false;
    	if (checkNumber.arguments.length == 4) ldefaultEmtpyOK = emptyOK;
	valid = checkString(fld, s, ldefaultEmtpyOK);
	if (valid && !isEmpty(fld.value)) {
		valid = isInteger(fld.value);
		if (!valid) {
			alert("Please enter numeric value in " + s + ".");
			fld.focus();
		}
		if (valid) {
			if (fld.value.length != size) {
				valid = false;
				alert(s + " must have " + size + " digits.");
				fld.focus();
			}
		}
	}
...
function checkYesCheckbox()
{
	var frm1 = document.forms[0];
		if (frm1.YesCheckbox.checked == false)
		{
		alert ('Please check Yes if you wish to attend.');
		return false;
		}
	return true;
}
